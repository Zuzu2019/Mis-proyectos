<?php
    ...
    if(!empty($pelic)){
        ...
        if($userid==1){
            $sql="SELECT p.titulo, p.duracion, p.rutaportada, g.nombre, c.clave FROM pelicula p, genero g, clasificacion c WHERE p.idclasificacion <= {$nivrest} AND p.eliminado=0 AND g.id=p.idgenero AND c.id=p.idclasificacion AND g.id=p.idgenero  AND (p.titulo LIKE '%{$busq}%' OR g.nombre LIKE '%{$busq}%')";
        }
        else{
            $sql="SELECT p.titulo, p.duracion, p.rutaportada, g.nombre, c.clave FROM pelicula p, genero g, clasificacion c WHERE p.idregion={$idregion} AND p.idclasificacion <= {$nivrest} AND p.eliminado=0 AND g.id=p.idgenero AND c.id=p.idclasificacion AND g.id=p.idgenero AND (p.titulo LIKE '%{$busq}%' OR g.nombre LIKE '%{$busq}%')";
