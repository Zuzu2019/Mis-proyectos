<?php
    ...
    if(!empty($pelic)){
        ...
        if($idregion==1){
            $sql="SELECT * FROM pelicula WHERE idclasificacion <= {$nivrest} AND eliminado=0";
        }
        else{
            $sql="SELECT * FROM pelicula WHERE (idregion={$idregion} OR idregion=1) AND idclasificacion <= {$nivrest} AND eliminado=0";
