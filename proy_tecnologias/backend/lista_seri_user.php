<?php
    ...
    if(!empty($pelic)){
        ...
        if($idregion==1){
            $sql="SELECT * FROM serie WHERE idclasificacion <= {$nivrest} AND eliminado=0";
        }
        else{
            $sql="SELECT * FROM serie WHERE (idregion={$idregion} OR idregion=1) AND idclasificacion <= {$nivrest} AND eliminado=0";
        }
